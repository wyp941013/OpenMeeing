...
<ul id="tabList">
<li id="xsdy_tabs" class="tabs" onclick="TabSwitch('zyjy','xsdy_tabs')">新生动员</li>
<li id="jstl_tabs" class="tabs" onclick="TabSwitch('jstl','jstl_tabs')">技术讨论</li>
<li id="bmlh_tabs" class="tabs" onclick="TabSwitch('bmlh','bmlh_tabs')">部门例会</li>
</ul>
</div>	
<div class="container div_table" style="float:left;position:relative;left:-270px;">
	...
	<table border="1" class="table table-striped table-bordered" id="zyjy" style="display:block">
		...
		<c:forEach  items="${listxsdy}" var="item">
		<tr>
	...
	<table border="1" class="table table-striped table-bordered" id="jstl">
		...
		<c:forEach  items="${listjstl}" var="item">
		<tr>
			<td>${item.meetTitle} </td>
			<td>${item.meetManager} </td>
			<td>${item.meetTime}</td>
	...
	<table border="1" class="table table-striped table-bordered" id="bmlh">
		...
		<c:forEach  items="${listbmlh}" var="item">
		<tr>
			<td>${item.meetTitle} </td>
			<td>${item.meetManager} </td>
			<td>${item.meetTime}</td>
